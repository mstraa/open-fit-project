...
## Roadmap (Helio-first)
- **M0 — Plugin scaffold + native GATT queue + relay (~1 wk).** Kotlin plugin, scan/connect,
  serialized op queue, CCCD notify, JS bridge, relay to `/api/wellness`. Prove with standard
  `0x180D` HR through the native path (native live-HR tile works).
- **M1 — Helio auth + encrypted transport + live HR (~2–3 wks, THE crux).** Port the auth
  handshake (ECDH-B163+AES *or* Xiaomi HMAC+AES-CCM, per the check above) + chunked
  encoder/decoder **verbatim**; add a known-answer test from a real handshake capture.
  Deliver: enter the auth key → live HR over the encrypted link, no Gadgetbridge app.
