...
- **Next**: the Gadgetbridge **DB reader/mapper** (exported `gadgetbridge` SQLite → wellness/activities → `POST /api/wellness` / import) — **needs a real GB export** to map the device-specific sample tables (Helio/Huami); then a Settings "server URL" editor; then BLE (2b) after the hardware coverage gate.
