...
**Current phase:** Phases 0–3 DONE ✅ · **Phase 4 (life tracker / wellness views)** 🚧 (Dashboard + Wellness + Sleep wired to real data; Trends/posture next) · **Phase 8 (MCP server) v1 DONE ✅** (pulled forward as a dev/debug aid)

## Phase 8 — MCP server (ofit-mcp + ofit-api) — v1 DONE ✅ (2026-06-04)
Embedded MCP server (PLAN.md "couche fine au-dessus de l'API") so LLM clients (Claude Code) can query and drive OpenFit — pulled forward from its "tardif" slot because it makes debugging/feature work materially easier. See **docs/MCP.md**.
- **ofit-mcp** (was a stub): rmcp 1.7 (official Rust MCP SDK, streamable-HTTP server transport) exposing **22 tools** in the Phase-8 families — orientation (`get_overview`, `db_schema`), catalog (`list_algorithms`, `list_variants`, `get_parameters`), query (`list_activities`, `get_activity`, `query_wellness`, `get_derived`, `get_training_load`, `list_sources`, `personal_records`, `get_gear`, `list_preferences`, `get_settings`), safe mutations (`recompute`, `set_parameters`, `set_selection`, `set_setting`, `set_preference`), escape hatches (`query_sql` read-only w/ keyword guard + streamed row cap; `api_request` GET/POST/PUT passthrough w/ imports/maintenance/auth/ingest/DELETE blocked). API-backed tools **self-dispatch in-process** (tower `oneshot` into a pre-auth, state-applied clone of the protected router) — zero handler duplication. Unbounded endpoints are shaped MCP-side: `list_activities` filters+paginates, `query_wellness` defaults to last-7-days + stride-downsample, `get_activity` re-downsamples with metric filtering (true counts always reported).
- **ofit-api**: the terminal `.route_layer(require_auth)` is split off `protected_api` so the MCP dispatcher captures the pre-auth router; `/mcp` is mounted via `nest_service` **behind the same `require_auth`** (session cookie or `OFIT_TOKEN` bearer = the trust boundary; rmcp's localhost-only Host validation disabled deliberately). SPA fallback now 404-guards `/mcp`; global CORS allows/exposes `mcp-session-id` (+ protocol headers) for browser clients.
- **Deps**: workspace `rmcp 1.7` / `schemars 1` / `serde_urlencoded` / `futures-util`; `rust-version` bumped 1.80 → **1.85** (rmcp is edition 2024). No conflicts (rmcp pulls neither axum nor hyper).
- **Verified end-to-end**: `cargo check/test --workspace` green (11 new unit tests: SQL guards, stride sampler, ts normalization, enum-mirror drift). Live: empty-DB + **2.17 GB real-data copy** with `OFIT_TOKEN` — full streamable-HTTP lifecycle via curl (initialize → `Mcp-Session-Id` → initialized 202 → tools/list 22 → tools/call → SSE GET stream → DELETE 202) **and** via the official `@modelcontextprotocol/inspector` CLI; 401 without credentials; write-SQL and blocked-path attempts rejected; mutations round-trip (`set_setting`, API 400 surfaced as readable tool error). Repo `.mcp.json` registers `openfit` for Claude Code (`Authorization: Bearer ${OFIT_TOKEN}`).
- **Hardened after an adversarial multi-agent review** (16 confirmed findings applied, all re-verified live): `/mcp` uses a new `require_auth_strict` (NEVER first-run-open — the wizard only needs `/api/auth/*`; verified: fresh install → `/api` 200, `/mcp` 401); `query_sql` enforces read-only at the **engine** level (detached conn + `PRAGMA query_only`/`default_transaction_read_only`), denies the auth tables (`users`/`sessions` — live bearer tokens must not enter an LLM context), caps cells (64 KB) and total bytes (4 MB), and its keyword list no longer false-rejects `REPLACE()`/`set` columns; `kind`/`metric` params are schemars **enum mirrors** of the core serde enums (sp_o2, race_predict5k, lat_lng… advertised in the schema; drift-guarded by round-trip tests); `get_derived` now downsamples stream points (true `sample_count` kept — the CTL/ATL/TSB streams are ~4k points each); over-cap responses (>64 MiB, e.g. multi-year minute-HR windows) return a readable narrow-the-window tool error instead of an opaque RPC failure; `list_activities` compares timestamps chronologically (not lexically) and reports `has_more`; every tool carries MCP annotations (`readOnlyHint` etc.); all 22 tool schemas verified live. **Deploy unbroken**: Dockerfile builder bumped `rust:1.83`→`1.87` (rmcp is edition 2024 — 1.83 cannot compile it), LXC update script now `rustup update stable` on existing containers, `Cargo.lock` is **committed** (removed from .gitignore) and Docker/LXC/CI builds use `--locked`; optional `OFIT_CORS_ORIGINS` allowlist (default keeps reflect-origin).
- **Next**: Phase 9 `viz_*` tools (VisualizationSpec creation) on this server; optional MCP resources (OpenAPI doc); graceful shutdown for in-flight SSE streams; Postgres-backend pass over `query_sql` cell decoding (NUMERIC/JSONB land as explicit `<undecoded:…>` markers today).
...
- **Zepp / Amazfit app-export import — DONE ✅** `ofit-ingest::zepp` reads the app's "Export data" folder (zipped) — `read_zepp_zip(bytes)` extracts + `read_zepp_export(dir)` parses the category CSVs: `HEARTRATE_AUTO`→HeartRate, `SLEEP_MINUTE`→**SleepStage** (+in-sleep HR & respiration), `ACTIVITY`(daily)→Steps+**Calories**, `BODY`→**Weight**, and **`SPORT`→summary activities**. New `WellnessKind::{Weight,Calories}`. `POST /api/import/zepp` (multipart .zip) + a Wellness-screen card. Verified on the real export: **772,882 readings** + **368 workouts**.
- **Zepp SPORT → summary activities — DONE ✅** workout summaries (no streams/GPS — just totals) import as **stream-less recordings** whose stats live in `metadata` (`summary_only:true`, distance/calories/avg-pace + raw zepp type). One activity each, idempotent by a stable `zepp-sport:{type}:{ts}:{dist}` hash. `ActivityDetail.summary` (new `ActivitySummaryStats`) surfaces distance/pace/calories so the **WorkoutDetail** shows summary tiles instead of a blank chart page (the GPS/stream cards keep their empty states). Sport mapping: 1/8→Running, 6/10→Cycling, 9→Walking, 52→Swimming, else Other.
- **Summary↔FIT dedup — DONE ✅** a Zepp summary that duplicates a real streamed activity (same sport + overlapping window) is suppressed: `import_zepp` loads real-activity windows once and skips overlapping summaries (`real_activity_windows`), and `cleanup_duplicate_summary_activities` removes pre-existing dups (runs at the end of every import = self-healing; also exposed one-shot at `POST /api/maintenance/dedup-zepp-summaries`). "Real" = an activity with ≥1 non-summary recording; cleanup only deletes the activity grouping (the `RawRecording` is preserved → idempotent). Response carries `activities_skipped_dup` + `duplicate_summaries_removed`.

## Phase 4 — feature batch (auto-import, settings imports, layout, sport-aware, readiness)
- **Gadgetbridge hourly auto-import — DONE ✅ (foreground MVP)** `web/src/gadgetbridge/`: persisted config (`autoImportConfig.ts`: enabled + minute-of-hour + file path), `runImport.ts` (reads the DB via `@capacitor/filesystem` → `importGadgetbridge`, idempotent), `useGadgetbridgeAutoImport.ts` (30s-poll scheduler firing at the chosen minute + visibility/focus catch-up, per-hour dedupe) mounted once in `App.tsx`, and a Settings card (`GadgetbridgeAutoImportCard.tsx`: toggle, minute picker, path, "Import now", last-run). `@capacitor/filesystem` added to web deps; manifest gains `READ_EXTERNAL_STORAGE`/`MANAGE_EXTERNAL_STORAGE`. **Limitation:** foreground-only (timers freeze when the WebView is backgrounded/killed) — true background needs a native scheduler (`@capacitor/background-runner`/AlarmManager), noted in the UI.
- **Imports moved to Settings — DONE ✅** the manual Gadgetbridge-DB + Zepp-zip cards are now `web/src/ui/ImportCards.tsx`, rendered in a new Settings **"Imports & sync"** panel (with the auto-import card); removed from Wellness. The Dashboard's **"Import .FIT" topbar button was removed** (FIT import stays on the Activities page).
- **Readiness unified — DONE ✅** new `web/src/ui/ReadinessSection.tsx` (a real `.card` section, not the `.banner` notification) used identically on Dashboard + Wellness; single 4-tier scale (Primed/Balanced/Strained/Depleted), prominent score, HRV vs baseline, and a Recompute prompt when not yet computed.
- **Sport-aware workout detail — DONE ✅** the summary-activity tile now shows **Avg speed (km/h)** for cycling and **Avg pace (/km)** for run/walk (via `speedModeFor`); streamed activities were already sport-aware.
- **Layout — DONE ✅** the earlier `html,body{overflow-x:hidden}` guard had broken `position:sticky` → switched to `overflow-x:clip`, so the rail + topbar are **pinned on scroll** again. Added `--topbar-h:64px` and set both the rail brand block and the topbar to that height so their **bottom borders align** into one line.

## Phase 4 — UI pass (open-to-dashboard, no Phase-X stubs, mobile-responsive)
- **Opens to Dashboard — DONE ✅** removed the Launcher home page; `/` → `<Navigate to="/dashboard">`, `*` → dashboard, deleted `Launcher.tsx` + `ComingSoon.tsx` and the `/trends` ComingSoon route + its rail nav item.
- **No "Phase X" widgets — DONE ✅** `EmptyState` no longer renders the roadmap `phase` tag (prop kept as ignored/deprecated so call sites still type-check); Dashboard `StatTileEmpty` shows "No data yet" not "No data · Phase N"; dropped the "Direct BLE (Phase 2b)" visible label. The screens were already wired to real data — only the stale tags were showing.
- **Mobile responsive / no horizontal scroll — DONE ✅** added a global overflow guard (`html,body{overflow-x:hidden}` + `overflow-x:clip` on `#root/.app/.main`), a 440px phone tier (tighter `--gap`/padding, 2-col stats), wrapped the activities `<table>` in `.tbl-wrap{overflow-x:auto}`, clamped uPlot chart containers (`max-width:100%;overflow:hidden`), made `LiveCard` wrap + its sparkline fluid, `.src` device pills truncate, `.banner` wraps, and `.summary`/`.alg-grid` collapse on phones (`minmax(min(100%,Npx),1fr)`). The drawer rail already collapsed at 920px — root cause was the missing overflow guard + no <620px tier.
...
- **BLE service discovery — DONE ✅** on connect we now `getServices()` and only subscribe to the standard measurement services actually present; if a device exposes **none** (the **Amazfit Helio** and full Garmin speak proprietary protocols — no standard HR GATT for third-party apps), we disconnect and show a clear message pointing to the Zepp/Gadgetbridge export path, instead of sitting "connected" with no data. Direct BLE remains for standard-GATT broadcasters (chest straps, watches in "broadcast HR", Stryd power).
