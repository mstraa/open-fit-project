...
- **Gadgetbridge import — DONE ✅** `ofit-ingest::gadgetbridge` reads an exported GB SQLite (Huami/Amazfit tables) → HR/steps/stress + derived daily resting-HR (auto-normalizes mixed s/ms epochs). `POST /api/import/gadgetbridge` (multipart) + a Wellness-screen upload card (works in browser + the app's webview). Verified on the real Helio export: **74,565 readings**.
- **Direct BLE (2b) — STARTED 🚧** `web/src/screens/BleDevices.tsx` (route `/devices`): scan nearby BLE devices (= the hardware-coverage gate, shows which standard services each exposes), connect, stream live **HR (0x180D) / power (0x1818, Stryd) / cadence (0x1814)** → live tiles + `POST /api/wellness`. `@capacitor-community/bluetooth-le` (app + Chrome Web Bluetooth); manifest BLE perms; **APK builds with it**. All BLE runtime is on-device (no BLE in CI). Proprietary device protocols stay on the GB-DB path until vendored.
- **Next**: confirm on-device which services the 945/Stryd/Helio expose; HRV source (the 945) so Readiness fills; Gadgetbridge auto-export auto-sync; a Settings "server URL" editor.
