...
Hardware support is delegated to **[Gadgetbridge](https://gadgetbridge.org/)** (BLE, local). We consume what Gadgetbridge exposes, plus an initial `.fit` import for backfill. No vendor cloud (Garmin Connect / Zepp / Stryd PowerCenter are deliberately excluded).
...
| `ofit-ingest` | Initial FIT import + Gadgetbridge bridge + batch/stream ingestion |
...
## License
**AGPL-3.0-or-later** for the server (network copyleft, fits self-hosted web); the mobile app will be GPLv3. Compatible with Gadgetbridge (GPL) and wger (AGPL).
